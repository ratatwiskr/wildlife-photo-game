...
export class Confetti {
  ...
  burst(count = 40) {
    ...
    for (let i = 0; i < count; i++) this.spawnPiece(true);
  }
  ...
  startContinuous(rate = 6) {
    ...
    this.running = true;
    const loop = () => {
      for (let i = 0; i < rate; i++) this.spawnPiece(false);
  ...
  private spawnPiece(fast = false) {
    ...
    el.style.background = ['#ff5c5c', '#ffd166', '#06d6a0', '#118ab2', '#9b5de5'][Math.floor(Math.random() * 5)];
